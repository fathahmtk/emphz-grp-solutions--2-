               ...
               <div className="flex space-x-8 min-w-max">
                  {[
                     ...
                     <button
                        ...
                        className={`py-6 text-xs font-bold uppercase tracking-widest border-b-2 transition-all ${activeTab === tab.id ? 'border-blue-600 text-slate-900' : 'border-transparent text-slate-400 hover:text-slate-600'}`}
                     >
                        {tab.label}
                     </button>
                  ))}
            ...
            {activeTab === 'comparison' && (
               <div className="animate-fade-in space-y-16">
                  <div className="text-center max-w-3xl mx-auto mb-12">
                     <h2 className="text-3xl font-semibold text-slate-900 mb-4 tracking-tight">Material Property Matrix</h2>
                     <p className="text-slate-600 font-light">Technical comparison of Fiber Reinforced Composites versus traditional metallic substrates.</p>
                  </div>

                  <div className="overflow-x-auto glass rounded-3xl border border-slate-200 shadow-2xl bg-white/40">
                     <table className="w-full min-w-[800px] text-sm font-mono">
                        <thead className="bg-slate-900 text-white">
                           <tr>
                              ...
                              <th className="p-6 text-center font-bold w-1/4 border-l border-white/5 text-slate-400">Mild Steel</th>
                              <th className="p-6 text-center font-bold w-1/4 border-l border-white/5 text-slate-400">Stain. Steel</th>
                           </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                           {[
                              ...
                              <tr key={i} className="hover:bg-blue-50/30 transition-colors">
                                 <td className="p-6 font-bold text-slate-900 bg-slate-50/50">{row.prop}</td>
                                 <td className="p-6 text-center font-bold text-blue-600 bg-blue-50/10 border-x border-slate-100">{row.grp}</td>
                                 <td className="p-6 text-center text-slate-500">{row.ms}</td>
                                 <td className="p-6 text-center text-slate-500">{row.ss}</td>
                              </tr>
                           ))}
                        </tbody>
                     </table>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                     <div className="glass p-8 rounded-2xl border border-slate-200 flex flex-col items-center text-center bg-white/60">
                        <Scale className="w-10 h-10 text-blue-600 mb-4" />
                        <h4 className="font-semibold text-lg text-slate-900 mb-2">Mass Reduction</h4>
                        <p className="text-sm text-slate-600 font-light">75% weight reduction facilitates rapid site deployment and reduced foundation requirements.</p>
                     </div>
                     <div className="glass p-8 rounded-2xl border border-slate-200 flex flex-col items-center text-center bg-white/60">
                        <ShieldCheck className="w-10 h-10 text-blue-600 mb-4" />
                        <h4 className="font-semibold text-lg text-slate-900 mb-2">Dielectric Barrier</h4>
                        <p className="text-sm text-slate-600 font-light">Inherent insulative properties eliminate the risk of step-and-touch voltage hazards.</p>
                     </div>
                     <div className="glass p-8 rounded-2xl border border-slate-200 flex flex-col items-center text-center bg-white/60">
                        <AlertTriangle className="w-10 h-10 text-blue-600 mb-4" />
                        <h4 className="font-semibold text-lg text-slate-900 mb-2">Low Salvage Value</h4>
                        <p className="text-sm text-slate-600 font-light">Unlike scrap-intensive metals, GRP discourages theft at remote, unmonitored installations.</p>
                     </div>
                  </div>
               </div>
            )}
               ...
               <div className="animate-fade-in">
                  <div className="flex flex-col lg:flex-row gap-16 items-center">
                     <div className="lg:w-1/2">
                        ...
                        <h2 className="text-3xl md:text-4xl font-semibold text-slate-900 mb-6 tracking-tight">CapEx vs. OpEx Analysis.</h2>
                        ...
                        <div className="glass p-6 rounded-xl border-l-4 border-blue-600 bg-white/60">
                           <div className="flex items-center gap-4 text-slate-900 font-bold text-xl mb-1">
                              <TrendingUp className="text-blue-600" /> +300% Life-Cycle ROI
                           </div>
                           <p className="text-sm text-slate-500 font-light">Cumulative savings calculated over a 20-year operational window in industrial environments.</p>
                        </div>
                     </div>

                     <div className="lg:w-1/2 w-full">
                        <div className="bg-slate-900 p-8 rounded-3xl shadow-2xl relative overflow-hidden text-white h-[400px] flex flex-col border border-white/5">
                           <div className="absolute inset-0 bg-dots opacity-5"></div>
                           <h3 className="text-center font-bold uppercase tracking-widest text-[10px] text-slate-400 mb-8 relative z-10">Asset Cost Projection Index</h3>

                           <div className="flex-grow relative border-l border-b border-slate-700 mx-4 mb-4 z-10">
                              {/* Grid Lines */}
                              <div className="absolute top-1/4 left-0 w-full h-px bg-slate-800/50"></div>
                              <div className="absolute top-2/4 left-0 w-full h-px bg-slate-800/50"></div>
                              <div className="absolute top-3/4 left-0 w-full h-px bg-slate-800/50"></div>
                              ...
                              {/* Steel Line (Steep) */}
                              <div className="absolute bottom-0 left-0 w-full h-[85%] border-t-2 border-slate-500 transform origin-bottom-left skew-y-[-28deg] opacity-70"></div>
                              <div className="absolute top-0 right-0 transform translate-x-2 text-slate-400 font-bold text-[10px] font-mono">METALLIC_STD</div>

                              {/* Crossover Point */}
                              <div className="absolute bottom-[28%] left-[22%] w-3 h-3 bg-blue-600 rounded-full border-2 border-slate-900 z-10 shadow-lg shadow-blue-500/50"></div>
                           ...
                           <div className="flex justify-between text-[9px] text-slate-500 px-4 font-mono relative z-10 uppercase tracking-widest">
                              <span>Init</span>
                              <span>Y-05</span>
                              <span>Y-10</span>
                              <span>Y-15</span>
            ...
            {activeTab === 'chemical' && (
               <div className="animate-fade-in max-w-4xl mx-auto">
                  <div className="text-center mb-12">
                     ...
                     <h2 className="text-3xl font-semibold text-slate-900 mb-4 tracking-tight">Chemical Resistance Index</h2>
                     <p className="text-slate-600 font-light">The chemical integrity of a composite system is governed by its resin matrix. Query our database for performance ratings against specific media.</p>
                  </div>

                  <div className="glass p-8 md:p-12 rounded-3xl border border-slate-200 shadow-xl bg-white/60">
                     <div className="relative max-w-xl mx-auto mb-10">
                        <input
                           ...
                           className="w-full pl-6 pr-14 py-4 rounded-2xl border border-slate-200 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none text-base shadow-sm bg-white"
                        />
                        <button
                           onClick={handleSearch}
                           className="absolute right-2 top-2 bottom-2 w-12 bg-slate-950 text-white rounded-xl flex items-center justify-center hover:bg-blue-600 transition-colors"
                        >
                           <Search size={20} />
                        </button>
                     </div>
                     ...
                     {chemicalResult && (
                        <div className="bg-white rounded-2xl p-6 md:p-8 border border-slate-100 shadow-xl animate-fade-in">
                           <div className="flex justify-between items-start mb-6 border-b border-slate-50 pb-4">
                              <h3 className="text-xl font-semibold text-slate-900">{chemicalResult.chemical}</h3>
                              <span className={`px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${chemicalResult.rating === 'Excellent' ? 'bg-green-50 text-green-700 border border-green-100' : chemicalResult.rating === 'Good' ? 'bg-blue-50 text-blue-700 border border-blue-100' : 'bg-slate-100 text-slate-600 border border-slate-200'}`}>
                                 {chemicalResult.rating}
                              </span>
                           </div>
                           <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                              <div>
                                 <span className="text-[10px] text-slate-400 uppercase font-bold tracking-[0.2em] block mb-2">Recommended Matrix</span>
                                 <div className="text-blue-600 font-semibold text-lg">{chemicalResult.resin}</div>
                              </div>
                              <div>
                                 <span className="text-[10px] text-slate-400 uppercase font-bold tracking-[0.2em] block mb-2">Technical Commentary</span>
                                 <div className="text-slate-600 text-sm leading-relaxed font-light italic">"{chemicalResult.note}"</div>
                              </div>
                           </div>
                        </div>
                     )}
                     ...
                     {!chemicalResult && (
                        <div className="text-center text-slate-400 text-xs tracking-wide py-8 font-mono">
                  ...
                  <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
                     <div className="p-6 glass rounded-2xl border border-slate-100 bg-white/40">
                        <div className="font-semibold text-slate-900 mb-2 uppercase text-xs tracking-widest">Orthophthalic</div>
                        <p className="text-xs text-slate-500 font-light leading-relaxed">General purpose systems. Optimal for water management and light atmospheric exposure.</p>
                     </div>
                     <div className="p-6 glass rounded-2xl border border-blue-100 bg-blue-50/10">
                        ...
                        <p className="text-xs text-slate-600 font-light leading-relaxed">High-performance water and corrosion barrier. Our default for coastal infrastructure.</p>
                     </div>
                     <div className="p-6 glass rounded-2xl border border-slate-100 bg-white/40">
                        <div className="font-semibold text-slate-900 mb-2 uppercase text-xs tracking-widest">Vinyl Ester</div>
                        <p className="text-xs text-slate-500 font-light leading-relaxed">Premium industrial grade. Engineered for aggressive acids, alkalis, and thermal loads.</p>
                     </div>
                  </div>
               </div>
            )}
