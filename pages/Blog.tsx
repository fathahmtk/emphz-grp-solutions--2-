    ...
    <div className="bg-emphz-navy text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24">
          ...
          <Link to={`/blog/${featuredPost.slug}`} className="group block md:grid grid-cols-2 gap-10 items-center">
            ...
            <div>
              ...
              <div className="flex items-center text-sm font-bold text-emphz-teal group-hover:text-white transition-colors">
                Read Full Article <ArrowRight size={16} className="ml-2 group-hover:tranneutral-x-1 transition-transform" />
              </div>
            </div>
          </Link>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {otherPosts.map(post => (
            <Link key={post.slug} to={`/blog/${post.slug}`} className="group bg-white/5 rounded-2xl overflow-hidden border border-white/10 hover:border-emphz-teal/50 transition-all duration-300 hover:-tranneutral-y-1 hover:shadow-2xl hover:shadow-emphz-teal/10">
