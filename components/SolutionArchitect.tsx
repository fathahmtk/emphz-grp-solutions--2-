    ...
    <section id="solutions" className="py-24 md:py-32 bg-white text-emphz-darker overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <div className="text-center mb-20">
           <span className="font-mono text-emphz-teal text-xs font-bold tracking-[0.4em] uppercase mb-4 block">14 / COMPOSITE VETTING</span>
           <h2 className="text-4xl md:text-6xl font-bold text-emphz-darker tracking-tighter uppercase mb-6">GRP <span className="text-emphz-teal">Architect.</span></h2>
           <p className="text-slate-500 max-w-2xl mx-auto font-light leading-relaxed text-lg">
             Our protocol matches site environmental stressors with the precise GRP composite configuration required for sustained performance.
           </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-12">
           {ENVIRONMENTS.map((env) => (
             <button
               ...
               className={`p-10 text-center rounded-[2.5rem] transition-all duration-500 relative group border ${
                 activeEnv.id === env.id 
                 ? 'bg-emphz-darker text-white border-emphz-darker shadow-2xl scale-[1.02]' 
                 : 'bg-slate-50 text-slate-400 border-slate-100 hover:border-emphz-teal/30 hover:bg-white'
               }`}
             >
                ...
                <p className={`text-[9px] font-mono uppercase tracking-widest ${activeEnv.id === env.id ? 'text-emphz-teal' : 'text-slate-500'}`}>
                  {env.stress}
                </p>
             </button>
           ))}
        ...
        <div className="bg-slate-50 rounded-[3rem] p-12 md:p-20 border border-slate-100 animate-in fade-in slide-in-from-bottom-8 duration-700" key={activeEnv.id}>
           <div className="flex flex-col lg:flex-row gap-20 items-center">
              ...
              <div className="lg:w-2/3">
                 <p className="text-slate-400 font-mono text-[10px] uppercase tracking-[0.3em] mb-10 font-bold">Vetted GRP Bill of Materials (BOM)</p>
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {activeEnv.solutions.map((sol, i) => (
                      <div key={i} className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 group hover:shadow-xl transition-all duration-500 flex items-center justify-between">
                         ...
                         <ArrowRight className="w-4 h-4 text-slate-200 group-hover:text-emphz-teal transition-colors" />
                      </div>
                    ))}
