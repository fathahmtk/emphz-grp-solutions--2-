...
import { Beaker, CheckCircle2, AlertCircle, Activity } from 'lucide-react';
              ...
              <div className="grid gap-2">
                {(['GRP', 'Steel', 'Concrete'] as Material[]).map((m) => (
                  <button
                    ...
                    className={`p-4 border font-mono text-xs tracking-widest text-left flex justify-between items-center transition-all ${activeMaterial === m ? 'bg-emphz-teal text-white border-emphz-teal' : 'bg-white/5 text-neutral-400 border-white/5 hover:border-white/20'
                      }`}
                  >
                    {m} UNIT_TEST
              ...
              <div className="flex gap-2">
                {(['Corrosion', 'UV', 'Impact'] as Stress[]).map((s) => (
                  <button
                    ...
                    className={`flex-1 p-4 border transition-all flex flex-col items-center gap-2 ${activeStress === s ? 'bg-emphz-teal/20 text-emphz-teal border-emphz-teal' : 'bg-white/5 text-neutral-500 border-white/5 hover:border-white/20'
                      }`}
                  >
