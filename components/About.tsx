      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <div className="flex flex-col lg:flex-row gap-20 items-center">
          ...
          <div className="lg:w-1/2">
            <SectionHeader
              number="01"
              category="Corporate Integrity"
              title="Engineering Future Resilience"
              subtitle="EMPHZ bridges the gap between traditional heavy construction and modern composite science. We specialize in precision-engineered GRP systems that eliminate corrosion risks and accelerate site infrastructure readiness."
              light={true}
            />

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12">
              {[
                { title: "Material Inertia", desc: "Completely immune to hydrogen sulfide and offshore salinity." },
                { title: "Rapid Deploy", desc: "Modular architecture reduces site-work duration by up to 70%." }
              ].map((item, i) => (
                <GlowCard key={i} className="p-8 bg-white border border-neutral-100 rounded-[2rem] shadow-sm">
                  <h4 className="text-emphz-blue font-bold uppercase tracking-widest text-xs mb-3">{item.title}</h4>
