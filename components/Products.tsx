...
const ProductAccordion: React.FC<{
  ...
  return (
    <div ref={containerRef} className="border-b border-neutral-100 last:border-0 scroll-mt-24">
      <button
        onClick={onToggle}
        className="w-full py-4 flex items-center justify-between group"
      >
        <div className="flex items-center gap-3">
          <div className={`transition-colors duration-300 ${isOpen ? 'text-emphz-blue' : 'text-neutral-400 group-hover:text-emphz-blue'}`}>
            {icon}
          </div>
          <span className={`text-[10px] font-bold tracking-[0.2em] uppercase transition-colors duration-300 ${isOpen ? 'text-emphz-blue' : 'text-neutral-500 group-hover:text-emphz-blue'}`}>
            {title}
          </span>
        </div>
        <div className={`transition-transform duration-500 ${isOpen ? 'rotate-180 text-emphz-blue' : 'text-neutral-300'}`}>
  ...
  return (
    <section id="products" className="py-24 md:py-32 bg-white text-emphz-blue relative">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        <SectionHeader
          number="02"
          category="GRP CATALOGUE"
          title="Vetted Infrastructure Systems"
          subtitle="The EMPHZ ecosystem provides industrial GRP modules pre-qualified for the most demanding global utilities. Built for zero corrosion and designed for immediate site integration."
          align="center"
          light={true}
        />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 lg:gap-8">
          {filtered.map((product) => (
            <GlowCard
              key={product.id}
              className="group bg-white rounded-[3rem] shadow-[0_40px_100px_-30px_rgba(0,0,0,0.06)] border border-neutral-100 p-8 hover:-translate-y-2 hover:shadow-2xl transition-all duration-700 flex flex-col h-full"
            >
              <div className="relative aspect-[4/5] rounded-[2.5rem] overflow-hidden mb-10 shadow-xl shrink-0">
                ...
                <div className="absolute top-8 left-8 w-16 h-16 bg-white/90 backdrop-blur-xl rounded-full shadow-2xl flex items-center justify-center text-emphz-blue border border-white/50">
                  {product.icon}
                </div>
              </div>
              ...
              <div className="px-2 flex flex-col flex-grow">
                ...
                <h3 className="text-2xl font-bold text-emphz-blue mb-4 uppercase tracking-tight group-hover:text-neutral-500 transition-colors">
                  {product.title}
                </h3>

                <p className="text-neutral-500 text-sm leading-relaxed mb-6 line-clamp-2 font-light">
                  {product.description}
                </p>

                <div className="mb-10 bg-neutral-50/50 rounded-2xl px-6 py-2 border border-neutral-100">
                  <ProductAccordion
                    title="Key Features"
                  ...
                  >
                    <ul className="space-y-3">
                      {product.features.map((f, i) => (
                        <li key={i} className="flex items-center gap-3 text-[10px] text-neutral-500 font-mono uppercase tracking-widest">
                  ...
                  >
                    <div className="space-y-3">
                      {Object.entries(product.specs).map(([key, val], i) => (
                        <div key={i} className="flex justify-between items-center text-[9px] font-mono uppercase tracking-widest border-b border-neutral-100/50 pb-2 last:border-0">
                          <span className="text-neutral-400 font-bold">{key.replace('_', ' ')}</span>
                  ...
                  >
                    <div className="space-y-4">
                      <div className="bg-white/50 p-3 rounded-lg border border-neutral-100">
                        <div className="flex items-center gap-2 mb-2">
                          <ShieldCheck className="w-3 h-3 text-emphz-blue" />
                          <span className="text-[9px] font-mono text-neutral-400 uppercase tracking-widest font-bold">Laminate Matrix</span>
                        </div>
                        <p className="text-[10px] text-neutral-600 font-medium leading-relaxed">{product.materialProperties}</p>
                      </div>
                      <div className="space-y-2">
                        {product.performanceData.map((data, i) => (
                          <div key={i} className="flex items-center gap-3 text-[9px] text-neutral-500 font-mono uppercase tracking-widest bg-white p-2 border border-neutral-100 rounded">
