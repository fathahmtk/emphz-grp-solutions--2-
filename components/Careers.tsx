...
import { Briefcase, ArrowRight, Users, MapPin, Search, ChevronRight } from 'lucide-react';

const Careers: React.FC = () => {
  return (
    <section id="careers" className="py-24 md:py-32 bg-white text-emphz-darker relative overflow-hidden border-t border-slate-100">
        ...
        <div className="flex flex-col lg:flex-row gap-20 items-stretch">
           ...
           <div className="lg:w-1/2 flex flex-col justify-center">
             <div className="mb-12">
                ...
                <div className="inline-flex items-center gap-4 px-8 py-4 bg-slate-50 rounded-full border border-slate-100 mb-12">
                   ...
                   <div>
                      ...
                      <span className="text-[9px] text-slate-400 font-mono uppercase tracking-widest font-bold">Active Technical Openings</span>
                   </div>
                </div>
             </div>
             ...
             <div className="space-y-4">
                {CAREERS_DATA.map((job, idx) => (
                  <div key={idx} className="group p-8 bg-slate-50 rounded-[2.5rem] border border-slate-100 hover:bg-white hover:border-emphz-teal hover:shadow-2xl transition-all duration-700 cursor-pointer flex items-center justify-between">
                     <div className="flex items-center gap-8">
                        <div className="text-[10px] font-mono text-slate-300 uppercase font-bold group-hover:text-emphz-teal transition-colors">{job.id}</div>
                        <div>
                           ...
                           <div className="flex items-center gap-4">
                              <span className="flex items-center gap-1.5 text-[9px] font-mono text-slate-400 uppercase font-bold tracking-widest"><MapPin className="w-3 h-3" /> {job.location}</span>
                     ...
                     <div className="w-12 h-12 rounded-full border border-slate-200 flex items-center justify-center text-slate-300 group-hover:bg-emphz-teal group-hover:border-emphz-teal group-hover:text-white transition-all duration-500">
             ...
             <div className="mt-12 flex items-center gap-10">
               ...
               <p className="text-[9px] font-mono text-slate-400 uppercase tracking-widest font-bold max-w-[150px]">Priority response for CAD/Composite Exp.</p>
             </div>
           </div>

           {/* Right: Visual Storytelling */}
           <div className="lg:w-1/2 relative min-h-[600px] flex items-center justify-end">
              <div className="relative w-full aspect-[4/5] max-w-md rounded-[3.5rem] overflow-hidden border-[12px] border-slate-50 shadow-2xl group">
                <img 
                ...
                <div className="absolute bottom-12 left-12 right-12">
                   <div className="bg-white/10 backdrop-blur-xl border border-white/20 p-10 rounded-[2.5rem] shadow-2xl">
                      ...
                      <span className="text-white/60 text-[10px] font-mono text-slate-400 uppercase tracking-widest block font-bold">— Managing Director, EMPHZ</span>
                   </div>
                </div>
              </div>
              
              {/* Decorative Circle Background */}
              <div className="absolute -top-12 -right-12 w-64 h-64 bg-slate-50 rounded-full -z-10"></div>
           </div>
