...
const ComplianceMatrix: React.FC = () => {
  ...
  return (
    <section id="compliance" className="py-24 md:py-32 bg-slate-50 border-t border-slate-100 relative">
      <div className="max-w-7xl mx-auto px-6 md:px-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
           {standards.map((std, i) => (
             <div key={i} className="bg-white p-10 rounded-[3rem] shadow-[0_30px_60px_-20px_rgba(0,0,0,0.05)] hover:shadow-2xl transition-all duration-700 border border-slate-50 group flex flex-col">
                <div className="flex items-center gap-5 mb-10">
                   <div className="w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center text-emphz-teal group-hover:bg-emphz-teal group-hover:text-white transition-all duration-500">
                      <ShieldCheck className="w-6 h-6" />
                   </div>
                   <div>
                      <h4 className="text-xl font-bold text-emphz-darker tracking-tight">{std.title}</h4>
                      <span className="text-[9px] font-mono text-slate-400 uppercase tracking-widest font-bold">{std.label}</span>
                   </div>
                </div>
                <p className="text-sm text-slate-500 leading-relaxed font-light mb-8 flex-grow">
                   {std.desc}
                </p>
                <button className="flex items-center gap-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest group-hover:text-emphz-teal transition-colors">
        ...
        <div className="mt-24 bg-emphz-darker rounded-[3rem] p-12 md:p-16 flex flex-col md:flex-row items-center justify-between gap-12 shadow-2xl relative overflow-hidden">
           ...
           <div className="flex items-center gap-10 relative z-10">
              ...
              <div>
                 ...
                 <p className="text-slate-400 font-mono text-[10px] uppercase tracking-widest font-bold">Multi-National Compliance Certification Ready</p>
              </div>
           </div>
